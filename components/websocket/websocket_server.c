...
int ws_server_send_binary_all(uint8_t *msg, uint64_t len)
{
	xSemaphoreTake(xwebsocket_mutex, portMAX_DELAY);
	int ret = ws_server_send_binary_all_from_callback(msg, len);
	xSemaphoreGive(xwebsocket_mutex);
	return ret;
}
...
int ws_server_send_binary_all_from_callback(uint8_t *msg, uint64_t len)
{
	int ret = 0;
	for (int i = 0; i < WEBSOCKET_SERVER_MAX_CLIENTS; i++)
	{
		if (ws_is_connected(clients[i]))
		{
			ws_send(&clients[i], WEBSOCKET_OPCODE_BIN, (char *) msg, len, 0);
			if (ws_is_connected(clients[i]))
				ret += 1;
			else
			{
